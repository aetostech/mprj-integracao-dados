...
class S3_to_Redshift:
    ...
    def __init__(self) -> None:
        self.redshift_full_table_name = "sistema_financeiro.bcb_instituicao_financeira"
        self.s3_uri = f's3://linker-etl/processed/instituicoes_financeiras/{datetime.today().strftime("%Y-%m-%d")}.csv'
    ...
    def insert_temp_table_data_to_main_table(self, cursor) -> None:
        ...
        WHERE EXISTS (SELECT * FROM temp WHERE cnpj_raiz = {self.redshift_full_table_name}.cnpj_raiz
        AND segmento = {self.redshift_full_table_name}.segmento);
