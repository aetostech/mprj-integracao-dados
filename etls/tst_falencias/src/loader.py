...
class S3_to_Redshift:
    ...
    def __init__(self) -> None:
        self.redshift_full_table_name = "tst.falencias"
        self.s3_uri = f's3://linker-etl/raw/tst_falencias/{datetime.today().strftime("%Y-%m-%d")}.csv'
    ...
    def insert_temp_table_data_to_main_table(self, cursor) -> None:
        ...
        WHERE EXISTS (SELECT * FROM temp
                      WHERE cnpj = {self.redshift_full_table_name}.cnpj
                        AND processo = {self.redshift_full_table_name}.processo
                        );
